...
function Pages(props: PagesProps) {
	...
	usePagesStore.getState().Event.on('next', ({ currentIndex, nextIndex }: any) => {
		api.start((i) => {
			if (i === currentIndex) {
	...
	usePagesStore.getState().Event.on('pre', ({ currentIndex, preIndex }: any) => {
		api.start((i) => {
			if (i === currentIndex) {
	...
	usePagesStore.getState().Event.on('navigationTo', ({ fromPageIndex, toPageIndex }: any) => {
		api.start((i) => {
			if (i < toPageIndex - 1 || i > toPageIndex + 1) return { display: 'none' };
	...
	const bind: any = useDrag(({ active, movement: [mx], direction: [xDir], cancel }) => {
		if (active && Math.abs(mx) > width / 2) {
			...
			const newIndex = _.clamp(index + (xDir > 0 ? -1 : 1), 0, pagesLength - 1);
	...
	return {
		PageWrap: {
			'&': {
				...
				touchAction: 'none',
				overflow: 'hidden',
				background: theme[props.themeType || 'primary']?.pages?.background || props.bgColor,
				...props.sx?.['&']
			}
		}
	};
